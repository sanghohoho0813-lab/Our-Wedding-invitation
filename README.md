...
### 재생 정책 — 왜 들어오자마자 소리가 나지 않는가

**소리가 나는 자동재생은 브라우저가 정책으로 막습니다.**
Chrome · Safari · 삼성인터넷 · 카카오톡 인앱 브라우저 모두 같고,
어떤 코드로도 우회할 수 없습니다. 하객이 화면을 한 번 건드려야 소리가 납니다.

그래서 할 수 있는 최선을 합니다.

1. 들어오자마자 **음소거 상태로** 곡을 재생해 둡니다. (음소거 재생은 허용됩니다)
2. 하객이 화면을 처음 건드리는 순간 **음소거만 풉니다.**
   이미 버퍼링이 끝나 있어서 그때 파일을 받기 시작하지 않고 바로 소리가 납니다.
   처음 듣는 순간이 곡 중간이 되지 않도록 **앞부분부터** 다시 들려줍니다.
3. 소리 켜기가 거부되면 다시 음소거로 돌려두고 **다음 제스처에서 또 시도**합니다.
4. 하객이 음악 버튼으로 직접 끄면 다시 켜지 않습니다.
5. 2.2초가 지나도 소리가 없으면 음악 버튼 옆에 `음악 켜기` 를 잠깐 보여줍니다.
   (`music.hint` 를 비우면 표시되지 않습니다)
...
> 터치를 가로채지 않고 화면에도 영향을 주지 않습니다.

진짜로 "들어가자마자 소리"가 필요하다면 방법은 하나뿐입니다 —
첫 화면에 `입장하기` 같은 버튼을 두고 그 탭을 제스처로 쓰는 것입니다.
(전문 청첩장 서비스들이 쓰는 방식입니다. 원하시면 넣어드릴 수 있습니다)
