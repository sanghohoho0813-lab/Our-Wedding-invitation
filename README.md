...
05 웨딩 인터뷰        버튼 → 모달   (기본 꺼짐)
06 예식 안내          일시·장소 + 사진 + 달력
07 D-DAY             남은 날짜
08 갤러리             3열 그리드 + 전체화면 뷰어
...
내용이 아직 없는 섹션(인터뷰 · 타임라인 · 함께한 시간)은 꺼둔 상태입니다.
...
### 6-1. ⚠️ 참석 여부 · 방명록 (지금은 로컬 저장)

`src/lib/backend.ts` 는 현재 **브라우저 로컬 저장소**에만 데이터를 넣습니다.
즉 **글을 쓴 사람 본인에게만 보이고, 신랑·신부에게 전달되지 않습니다.**

> **하객에게 링크를 보내기 전에 반드시**
> Supabase 를 연결하거나, `rsvp.enabled` / `guestbook.enabled` 를 `false` 로 꺼주세요.
...
- [ ] **참석 여부 · 방명록**: Supabase 연결 또는 기능 끄기 (6-1 참고)
- [ ] 신랑·신부 **연락처** 입력
- [ ] 혼주 **연락처** 입력
- [ ] **계좌번호** 입력
- [ ] **교통 안내** 확인
