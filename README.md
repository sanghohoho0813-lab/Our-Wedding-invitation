...
지금 남아 있는 곳은 두 군데입니다 —
**초대의 글**(`[예시]`)과 **셔틀버스 운행 시간**(`[확인 필요]`).
나머지 내용은 모두 두 분의 답변으로 채워졌습니다.
...
| 좋아하는 것 | `groom.likes` | 노래 부르기 · 웨이트 트레이닝 · AI와 놀기 |
| 지윤이 보는 상호 | `groom.partnerQuote` | 확정 |
| 상호가 보는 지윤 | `bride.partnerQuote` | 확정 |
| 초대의 글 | `invitation.body` | `[예시]` — **최종 문구 확인 필요** |
| 인용구 | `quote` | 곰돌이 푸 문장 |
| 웨딩 인터뷰 | `interview.qa` | 신랑 · 신부 답변 모두 확정 (6문항) |
| 서로에게 | `letters.items` | 신랑 · 신부 편지 모두 확정 |
| 타임라인 | `timeline.items` | 첫 만남 · 연인이 되던 날 · 결혼 |
| 일상 사진 | `dailyGallery.images` | 27장 — 타임라인 중간(`timeline.photosAfter`)에 펼쳐집니다 |
| 주차 · 화환 | `infoTabs.items` | 예식장 최종 안내 반영 |
...
**"우리의 시간"** 타임라인 중간(연인이 되던 날 → 결혼 사이)에 펼쳐집니다.
